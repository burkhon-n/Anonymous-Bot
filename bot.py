...
from config import TOKEN, ADMIN_ID, CHANNEL_ID, messages, json, get_blocked_users
import re
...
def message_handler(message: Message):
    ...
    if str(message.chat.id) in blocked_users:
        ...
        if contains_url(message.text):
            ...
            mess = bot.copy_message(CHANNEL_ID, message.chat.id, message.message_id)
...
def inline_handler(call: CallbackQuery):
    ...
    if data[0] == "approve":
        bot.copy_message(CHANNEL_ID, data[2], int(data[1]))
        ...
        bot.delete_message(CHANNEL_ID, int(data[1]))
